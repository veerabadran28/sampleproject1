...
# bedrock = boto3.client('bedrock', endpoint_url=BEDROCK_ENDPOINT_URL)
...
def get_context_list():
    ...
    return ["Prompt engineering basics"]
...
def get_text_response(model_id, temperature, template, context=None, user_input=None): #text-to-text client function
    ...
    if template == "":
        template = f"{context}. Provide list of areas the you can be of assistance based on the {context}. Show the areas you can help in a table and add a note as to examples of questions that users can ask you."

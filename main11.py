    ...
    if user_selection_option != "Code Generator":
        ...
        with st.expander("Data Insights"):
            ...
            with data_explanation_tab:
                if not data.empty:
                    data_explanation_response = call_claude3_api(f"Provide a summary of the following data: {data.head().to_dict(orient='records')}", max_chars=12000)
                    st.markdown(data_explanation_response, unsafe_allow_html=True)
                    #st.write(data_explanation_response)
